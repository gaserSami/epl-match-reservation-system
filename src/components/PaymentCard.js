/*
  This component is responsible for rendering the payment card and handling the payment process.
*/
...
const PaymentCard = (props) => {
  ...
  const isMatchClashing = async (UserIDD, MatchIDD) => {
    try {
      ...
      const userTickets = allTickets.filter(
        (ticket) => ticket.UserID._id === UserIDD
      );

      // Fetch the match data for the current match
      const currentMatchResponse = await axios.get(
        `http://localhost:5000/matches/${MatchIDD}`
      );
      ...
      for (let ticket of userTickets) {
        ...
        const ticketMatchResponse = await axios.get(
        ...
        if (
          currentMatchDate.toDateString() === ticketMatchDate.toDateString()
        ) {
          // If the match dates are the same, check if the match times overlap
          const currentMatchTime = new Date(
            `1970-01-01T${currentMatch.MatchTime}:00Z`
          );
          const ticketMatchTime = new Date(
            `1970-01-01T${ticketMatch.MatchTime}:00Z`
          );

          const matchDuration = 1.5 * 60 * 60 * 1000; // 1.5 hours in milliseconds

          if (
            currentMatchTime.getTime() <
              ticketMatchTime.getTime() + matchDuration &&
            currentMatchTime.getTime() + matchDuration >
              ticketMatchTime.getTime()
          ) {
            return true;
          }
        }
  ...
  const handleSubmit = (e) => {
    ...
    axios.get(`http://localhost:5000/tickets`).then((response) => {
      ...
      const ticketsForMatch = allTickets.filter(
        (ticket) => ticket.MatchID._id === MatchIDD
      );
      ...
      const isAnySeatReserved = mySeatsNumber.some((seat) =>
        allReservedSeats.includes(seat)
      );
  ...
  return (
    <div className="payment-card-container">
      <div className="ticket-details">
        <p>
          <b>Seats Number</b>: {mySeatsNumber.join(", ")}
        </p>
        <p>
          <b>Total Price</b>: {Pricee * mySeatsNumber.length}
        </p>
      </div>
      <form className="payment-card" onSubmit={handleSubmit}>
        <input
          type="text"
          placeholder="Card Number: 1234567812345678 "
          value={cardNumber}
        ...
        <input
          type="text"
          placeholder="Card Holder Name: Example"
