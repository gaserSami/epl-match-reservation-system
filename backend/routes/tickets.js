/*
  This file contains the routes for the Ticket model.
  The routes are mounted at /tickets.
  The routes use the Ticket model and Joi validation.
  The routes are exported for use in server.js.
 */

// requires: express, router, Ticket model, Joi
...
const ticketValidationSchema = Joi.object({
  ...
  SeatsNumber: Joi.array()
    .items(Joi.number().integer().min(1))
    .min(1)
    .required(), // Array of seat numbers with at least one element
...
const processQueue = async () => {
  while (ticketCreationQueue.length > 0) {
    ...
    try {
      ...
      const allTicketsForMatch = await Ticket.find({
      ...
      const isAnySeatReserved = requestedSeats.some((seat) =>
